...
export default function TabelaProdutos({ produtos, onUpdate, produtoEditado, setProdutoEditado }) {
  ...
  const totalCBM = produtos.reduce((acc, prod) => {
    const edit = produtoEditado[prod.id];
    const CTNS = Number(edit?.["CTNS"] ?? prod["CTNS"]);
    const CBM = parseFloat(prod["CBM"]) || 0;
    return CTNS > 0 ? acc + CTNS * CBM : acc;
  }, 0);
  ...
  const exportarProdutosValidos = () => {
    const produtosValidos = produtos.filter(prod => {
      const CTNS = Number(produtoEditado[prod.id]?.['CTNS'] ?? prod['CTNS']);
      return CTNS > 0;
    });
  
    const dataExportada = produtosValidos.map(prod => {
      ...
      const CBM = Number(prod["CBM"]);
      const CBM_TOTAL = CBM * CTNS;
    ...
    XLSX.writeFile(wb, "produtos-validos.xlsx");
  };
  ...
  <button
    onClick={exportarProdutosValidos}
    className="bg-blue-700 hover:bg-blue-800 text-white px-4 py-2 rounded text-sm shadow"
  >
    Exportar produtos 
  </button>
</div>
  ...
  onError={(e) =>
    (e.target.src = "https://nyc3.digitaloceanspaces.com/moribr/base-fotos/placeholder-image.jpg")
  }
  alt={prod.NAME}
  className="w-[250px] h-[200px] object-contain border cursor-pointer hover:opacity-80 transition"
/>

              <div className="flex-1 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-7 gap-2 text-xs whitespace-nowrap">
                {principais.map((campo) => (
                  <div key={campo} className="border rounded px-2 py-1 truncate" style={{ borderColor: '#222', borderWidth: '0.5px' }}>
  ...
  >
    ...
    <button
      onClick={() => setImagemSelecionada(null)}
      className="absolute top-4 right-4 bg-white text-black px-3 py-1 text-sm rounded shadow"
    >
      Fechar
    </button>
  </div>
            ...
            {expandidoId === prod.id && (
              <div className="mt-4 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2 text-xs">
                {todos.map((col) => {
                  ...
                  } else if (col === "CBM TOTAL") {
                    valor = CBM_TOTAL.toFixed(6);
                  } else if (col === "T.G.W") {
                    valor = (CTNS * (parseFloat(prod["G.W"]) || 0)).toFixed(2);
                  } else if (col === "T.N.W") {
                    valor = (CTNS * (parseFloat(prod["N.W"]) || 0)).toFixed(2);
                  } else {
                    valor = prod[col];
                  }

                  return (
                    <div key={col} className="border rounded px-2 py-1" style={{ borderColor: '#ccc', borderWidth: '0.3px' }}>
                      <span className="block font-semibold text-gray-700">{col}</span>
                      <div>{valor}</div>
                    </div>
                  );
                })}
